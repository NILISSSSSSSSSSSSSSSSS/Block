<template class="template">
    <div class="homeBox routerShow_width" v-loading.fullscreen.lock="fullscreenLoading">
...
<script>
    export default {
      ...
      mounted:function(){
        this.$http.get(this.config.bathUrl+"/echart")
          .then(res => {

            this.option.xAxis[0].data=res.data.data.x_data;
            this.option.series[0].data=res.data.data.series_data;
            this.init()
            })
          .catch(function (err) {
            console.log(err);
      ...
      watch:{
        //多语言切换
        "echartTitleText":function(now){
          this.option.title.text=now['echartTit']
          this.option.xAxis[0].name=now['echartXname']
          this.init()
       }
    },
      computed:{
        echartTitleText:function(){
          var echartTit={echartTit:this.$t('echartTit'),echartXname:this.$t('echartXname')}
          return echartTit;
        }
      },
      methods: {
